...
export default function AchievementsPage() {
  ...
  const handleSubmit = async () => {
    ...
    // Step 1: get signed upload URL (bypasses Vercel body limit)
    ...
    // Step 3: create achievement record
    const recordRes = await fetch("/api/achievements", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ category: panel, notes: form.notes.trim() || null, file_path: path }),
    });
    if (!recordRes.ok) {
    ...
    <div style={{ padding: "40px 32px", maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{ marginBottom: 36 }}>
        ...
        <p style={{ color: "rgba(255,255,255,0.4)", marginTop: 10, fontSize: 15, maxWidth: 560 }}>
          Track your wins. Upload proof and your coach will verify them.
        </p>
      </div>
          ...
          return (
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                ...
                {count > 0 && (
                  <span style={{ fontSize: 10, fontWeight: 700, color: "#00ff88", background: "rgba(0,255,136,0.1)", padding: "2px 7px", borderRadius: 4 }}>
                    {count}
                  </span>
                )}
              </div>
              ...
              return (
                ...
                >
                  <div style={{ minWidth: 0 }}>
                    <p style={{ color: "#f5f5f5", fontWeight: 600, fontSize: 14, margin: 0 }}>
                    ...
                    {a.admin_notes && a.status === "rejected" && (
                      <p style={{ color: "rgba(255,68,68,0.7)", fontSize: 12, margin: "4px 0 0" }}>
                        Coach note: {a.admin_notes}
                      </p>
                    )}
      ...
      {panel && (
        <div
          onClick={() => !uploading && setPanel(null)}
        ...
        >
          <div
            ...
            style={{ width: "100%", maxWidth: 480, background: "#0d0d0d", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 20, padding: "28px 24px" }}
          >
            ...
            <div
              ...
              style={{
                padding: "28px",
                borderRadius: 14,
                border: `2px dashed ${pendingFile ? "rgba(0,255,136,0.5)" : "rgba(255,255,255,0.1)"}`,
                background: pendingFile ? "rgba(0,255,136,0.04)" : "rgba(255,255,255,0.02)",
                textAlign: "center",
                cursor: "pointer",
                marginBottom: 16,
                transition: "all 0.2s",
            ...
            >
              {pendingFile ? (
                <>
                  <CheckCircle size={24} style={{ color: "#00ff88", margin: "0 auto 8px", display: "block" }} />
                  <p style={{ color: "#00ff88", fontWeight: 600, fontSize: 14, margin: "0 0 4px" }}>{pendingFile.name}</p>
                  <p style={{ color: "rgba(255,255,255,0.3)", fontSize: 12, margin: 0 }}>Click to change file</p>
                </>
              ) : (
                <>
                  <Upload size={24} style={{ color: "rgba(255,255,255,0.3)", margin: "0 auto 8px", display: "block" }} />
                  <p style={{ color: "#f5f5f5", fontWeight: 600, fontSize: 14, margin: "0 0 4px" }}>Click to upload</p>
                  <p style={{ color: "rgba(255,255,255,0.3)", fontSize: 12, margin: 0 }}>Images or PDFs accepted</p>
                </>
              )}
            </div>
            <input
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: "block", color: "rgba(255,255,255,0.45)", fontSize: 11, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", marginBottom: 6 }}>
                Notes (optional)
              </label>
              <textarea
                rows={2}
                ...
                placeholder="e.g. TopStep evaluation pass — $150k account"
                style={{ width: "100%", padding: "10px 13px", borderRadius: 9, background: "#181818", border: "1px solid rgba(255,255,255,0.1)", color: "#f5f5f5", fontSize: 13, outline: "none", resize: "vertical", boxSizing: "border-box" }}
              />
            </div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button
                ...
                style={{
                  ...
                  opacity: pendingFile && !uploading ? 1 : 0.5,
                }}
              >
              ...
              <button
                ...
                disabled={uploading}
                style={{ padding: "11px 18px", borderRadius: 10, background: "transparent", color: "rgba(255,255,255,0.4)", fontSize: 14, border: "1px solid rgba(255,255,255,0.1)", cursor: "pointer" }}
              >
                Cancel
              </button>
            </div>
